...
//!
//! Each runtime mode maps to one or more stable owner groups (`platform`,
//! `jobs`, `health`, and `eventlog`) and acquires the corresponding NATS KV lease
//! keys before opening owner-controlled SQLite stores or starting subsystems.
//! There is no standby or automatic reacquisition behavior.
//!
//! A lease carries an immutable acquisition fence for subsystem writes and a
//! separate current KV revision used by renewal and release. Multi-group
//! acquisition is verified concurrently under a bounded deadline. Renewal uses
