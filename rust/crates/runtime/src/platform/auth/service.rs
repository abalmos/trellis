...
use super::repository::{
    AuthorityReconciliationOutcome, AuthorizationMaterializationRepository, IssuanceSnapshot,
};
use super::{
    AuthorityState, AuthorityTarget, AuthorizationStateError, DesiredAuthorityRecord,
    IssuableAuthorizationState, MaterializationState, PrincipalKind, PrincipalState,
    RuntimeEvidence, SessionState,
};
...
#[derive(Clone, Debug)]
pub struct AuthorizationStateService<S> {
    repositories: S,
}
...
impl<S> AuthorizationStateService<S> {
    ...
    /// Borrow the underlying repository composite for internal mutation workflows.
    #[must_use]
    pub fn repositories(&self) -> &S {
        &self.repositories
    }
}
    ...
    S: AuthorizationMaterializationRepository,
{
    ...
    pub async fn resolve_issuable_state(
        &self,
        session_id: &str,
        now: i64,
