...
impl EventLogStore {
    ...
    pub fn initialize_schema(&self) -> Result<(), EventLogStoreError> {
        ...
        connection.execute_batch(
            ...
            CREATE INDEX IF NOT EXISTS idx_eventlog_events_publisher_participant ON eventlog_events (publisher_participant_id);
    ...
    pub fn contains_stream_sequence(
        &self,
        stream_sequence: u64,
    ) -> Result<bool, EventLogStoreError> {
        let stream_sequence = i64::try_from(stream_sequence)
            .map_err(|_| EventLogStoreError::SequenceOutOfRange(stream_sequence))?;
        let connection = self
            .connection
            .lock()
            .map_err(|_| EventLogStoreError::Poisoned)?;
        Ok(connection.query_row(
            "SELECT EXISTS(SELECT 1 FROM eventlog_events WHERE stream_sequence = ?1)",
            [stream_sequence],
            |row| row.get(0),
        )?)
    }
...
fn event_where_clause(filter: &EventLogFilter) -> (String, Vec<SqlValue>) {
    ...
    {
        clauses.push("(event_id LIKE ? OR subject LIKE ? OR owner_contract_id LIKE ? OR owner_event_name LIKE ? OR publisher_deployment_id LIKE ? OR publisher_participant_id LIKE ? OR trace_id LIKE ?)".to_string());
