...
# Keep the already-audited packet transform intact and layer the next root fix on
# top of its exact blob. This is scratch validation state only; none of these
# scripts land on rs.
BASE_TRANSFORM_BLOB = "4fc833179fa27288e08b87676a794ffbfa7db890"
base = subprocess.check_output(
    ["git", "cat-file", "blob", BASE_TRANSFORM_BLOB],
    text=True,
)
exec(compile(base, f"<blob {BASE_TRANSFORM_BLOB}>", "exec"), {})

# Rust ParticipantArtifactV1 normalization used to happen incidentally when
# authoring called the WASM resolver. With authoring now WASM-free, TypeScript
# must produce the same intrinsic normalized participant itself.
...
function normalizeParticipant(participant: JsonObject): JsonObject {
  ...
  if (queues) {
    for (const value of Object.values(queues)) {
      ...
      if (!queue) continue;
      for (const key of ["progress", "logs", "dlq"]) {
...
return_anchor = "  return participant;\n}\n\nfunction compareProtocolStrings"
if text.count(return_anchor) != 1:
    raise RuntimeError("compileParticipant return anchor changed")
text = text.replace(
    return_anchor,
