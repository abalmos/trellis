...
- `metric-catalog.json` — the normative metric catalog: instrument names,
  units, dimensions, and observation ownership.
- `runbooks.md` — the runbook for every reference alert. Alert annotations
  link to its headings.
...
Telemetry is diagnostic only. It never authorizes, gates readiness, or changes
a business result, and an unavailable Collector never affects Trellis work.
Native and built-in browser telemetry use separate Collector pipelines and
separate Prometheus jobs; browser reports are untrusted and never page as proof
that a server is healthy.
