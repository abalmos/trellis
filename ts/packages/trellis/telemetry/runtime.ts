...
async function initialize(
  ...
  try {
    ...
    // Install the native async context manager only when Trellis owns the
    // providers; a host that already registered one keeps its own.
    ...
    return {
      ...
      shutdown: async () => {
        ...
        try {
          ...
          console.warn("trellis telemetry: shutdown failed", error);
        }
      },
    };
  } catch (error) {
    console.warn("trellis telemetry: initialization failed", error);
    return {
      traces: ownedTraces,
      metrics: ownedMetrics,
      forceFlush: () => Promise.resolve(),
