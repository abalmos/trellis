...
Deno.test("TypeScript integration manifest conforms to shared matrix", async () => {
...
function buildConformanceReport(
  ...
  if (missing.length > 0) {
    messages.push(
      `missing TypeScript integration cases: ${missing.join(", ")}`,
    );
  }
  if (extra.length > 0) {
    messages.push(
      `extra TypeScript integration cases not in matrix: ${extra.join(", ")}`,
    );
  }
  if (localDuplicates.length > 0) {
    messages.push(
      `duplicate TypeScript integration case ids: ${
  ...
  if (fixturePrefixMismatches.length > 0) {
    messages.push(
      `TypeScript integration case ids with wrong fixture prefix: ${
