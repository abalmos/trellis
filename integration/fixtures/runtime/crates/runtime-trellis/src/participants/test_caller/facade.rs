...
impl<'a> Client<'a> {
    ...
    /// Access the `runtime` dependency alias facade.
    pub fn runtime(&self) -> uses::runtime::Client<'a> {
        uses::runtime::Client::new(self.inner)
    }
}
impl<'a> Service<'a> {
    ...
    /// Access the `runtime` dependency alias facade for outbound calls.
    pub fn runtime(&self) -> uses::runtime::Client<'a> {
        uses::runtime::Client::new(self.inner)
    }
}
