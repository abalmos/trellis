...
Existing live cases in `ts/integration/_support/cases.ts:9-468` already cover
...
These existing TypeScript live integration groups already cover foundational
public behaviors. Future deletion work should still map retired unit assertions
to the specific case ids or group note that proves equivalent coverage.

| Existing live coverage group                                     | Source range                               | Coverage note                                              |
| ---------------------------------------------------------------- | ------------------------------------------ | ---------------------------------------------------------- |
| RPC/events/operations/feeds/state/transfer/resources/jobs/health | `ts/integration/_support/cases.ts:10-261`  | Basic public client/service runtime workflows.             |
| Authority plan and service/app/device approval                   | `ts/integration/_support/cases.ts:262-433` | Approval and authority-plan basics.                        |
| SQL outbox                                                       | `ts/integration/_support/cases.ts:434-467` | Outbox commit, rollback, multi-event, and dispatch basics. |

## Unit Inventory Families

### TypeScript/Deno Families

- Result package: `ts/packages/result/tests/*`.
- Trellis test harness: `ts/packages/trellis-test/tests/*`, plus integration
...
- Auth public helpers: `ts/packages/trellis/auth/**/*.test.ts`.
- Contract support and package hygiene:
  `ts/packages/trellis/contract_support/**/*_test.ts`,
  `ts/packages/trellis/tests/*artifact*`, `*exports*`, `*subpaths*`, `*schema*`,
  `*import*`, and `*publishing*`.
- TypeScript service runtime:
  `ts/packages/trellis/service/runtime/**/*_test.ts`,
  `ts/packages/trellis/tests/operations*`, `rpc_integration_test.ts`,
  `trellis_api_guard_test.ts`, and `state_runtime_facade_test.ts`.
- UI/state tools: `ts/packages/trellis-svelte/**/*.test.ts`,
  `ts/apps/console/**/*.test.ts`, and `ts/portals/login/**/*.test.ts`.
...
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------- | ------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| First-admin bootstrap and `/bootstrap/client` duplicated happy-path assertions from `ts/services/trellis/auth/account_flows/bootstrap.test.ts` and `ts/services/trellis/auth/bootstrap/client.test.ts` | ts/deno          | duplicate-retired-live-control-plane | `control-plane.admin-bootstrap-creates-first-local-admin`, `control-plane.http-route-security-requires-admin-session`                                                                                                                                                | shrunk | `deno task -c ts/deno.json test:service-integration -- --case control-plane.admin-bootstrap-creates-first-local-admin` passed; `deno task -c ts/deno.json test:service-integration -- --case control-plane.http-route-security-requires-admin-session` passed; `deno test --no-check -A -c ts/deno.json ts/services/trellis/auth/account_flows/bootstrap.test.ts ts/services/trellis/auth/bootstrap/client.test.ts` passed.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | Removed only `completeAdminBootstrapLocalPassword creates the first active local admin`, `POST /bootstrap/client returns runtime bootstrap info for bound browser sessions`, and `POST /bootstrap/client returns auth_required when no bound user session exists`. Retained first-admin bootstrap edge/error units and `/bootstrap/client` exact-digest, inactive-user, non-client-contract, clock-skew, invalid-signature, known-inactive-digest units; retained CORS/header/parser/security/provider/crypto/schema coverage.                                                                      |
| Local-password account-flow HTTP completion happy-path assertions from `ts/services/trellis/auth/http/routes.test.ts`                                                                                  | ts/deno          | duplicate-retired-live-control-plane | `control-plane.password-reset-change-invalidates-old-password`                                                                                                                                                                                                       | shrunk | `deno task -c ts/deno.json test:service-integration -- --case control-plane.password-reset-change-invalidates-old-password` passed; `deno test --no-check -A -c ts/deno.json ts/services/trellis/auth/http/routes.test.ts` passed; `deno check -c ts/deno.json ts/services/trellis/auth/http/routes.test.ts` passed.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | Removed only `auth HTTP account-flow local-password endpoint completes target account flow`, a route success-path duplicate covered by the live password reset/change case. Retained password hashing parameter coverage, password reset/change RPC unit edge cases, invalid/expired/reset-token and inactive-target local-password route errors, local-login inactive/lockout/rate-limit errors, provider/CORS/cookie/parser/security/schema coverage, and bootstrap reset/session-revocation edge units.                                                                                          |
| Local-login app-session bind happy-path assertions from `ts/services/trellis/auth/http/routes.test.ts` and `ts/services/trellis/auth/session/bind.test.ts`                                             | ts/deno          | duplicate-retired-live-shared-matrix | `auth.local-login-binds-approved-client`                                                                                                                                                                                                                             | shrunk | `deno task -c ts/deno.json test:integration -- --case auth.local-login-binds-approved-client` passed; `cargo test --manifest-path rust/Cargo.toml -p trellis-rs --test integration auth_local_login_binds_approved_client -- --nocapture` passed; `deno test --no-check -A -c ts/deno.json ts/services/trellis/auth/http/routes.test.ts ts/services/trellis/auth/session/bind.test.ts` passed; `deno check -c ts/deno.json ts/services/trellis/auth/http/routes.test.ts ts/services/trellis/auth/session/bind.test.ts` passed.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | Removed only `auth HTTP local login creates pending auth for linked active identity` and `ensureBoundUserSession creates a new session when none exists`. Retained malformed bind/request validation, missing-identity invalid-credential behavior, local-login lockout/reset/inactive/expired-flow edge cases, rebinding/kick/session-authority-change/storage-error units, `Auth.Sessions.Me` envelope/active-account validation units, and provider/CORS/cookie/parser/security/schema/crypto coverage.                                                                                          |
| State admin inspect/list/delete happy-path assertions from `ts/services/trellis/state/rpc.test.ts`                                                                                                     | ts/deno and rust | duplicate-retired-live-shared-matrix | `state.admin-inspect-and-delete-state`                                                                                                                                                                                                                               | shrunk | `deno task -c ts/deno.json test:integration -- --case state.admin-inspect-and-delete-state` passed; `cargo test --manifest-path rust/Cargo.toml -p trellis-rs --test integration state_admin_inspect_and_delete_state -- --nocapture` passed; `deno test --no-check -A -c ts/deno.json ts/services/trellis/state/rpc.test.ts` passed; `deno check -c ts/deno.json ts/services/trellis/state/rpc.test.ts` passed.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Removed only duplicated `adminGet`, `adminList`, `adminDelete`, and post-delete missing success assertions from `State admin RPCs inspect and delete named stores`, plus the now-unused `listedKey` helper. Retained admin auth denial, missing-store validation, wrong-contract validation, migration metadata/schema coverage, session resolver failure coverage, ownership/validation/boolean schema units, and other state edge coverage.                                                                                                                                                       |
| Catalog accepted implementation offer active-catalog happy-path assertions from `ts/services/trellis/catalog/runtime.test.ts`                                                                          | ts/deno          | duplicate-retired-live-control-plane | `control-plane.catalog-active-contracts-survive-restart`, `control-plane.catalog-dependency-issue-resolved-by-provider`                                                                                                                                              | shrunk | `deno task -c ts/deno.json test:service-integration -- --case control-plane.catalog-active-contracts-survive-restart` passed; `deno task -c ts/deno.json test:service-integration -- --case control-plane.catalog-dependency-issue-resolved-by-provider` passed; `deno test --no-check -A -c ts/deno.json ts/services/trellis/catalog/runtime.test.ts` passed; `deno check -c ts/deno.json ts/services/trellis/catalog/runtime.test.ts` passed.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Removed only `contracts runtime includes accepted implementation offers as active`. Retained unknown dependency validation, optional/grouped uses, known inactive provider/use coverage, stale/expired offers, incompatible active-offer issue coverage, cache pruning/repair, dry-run mutation guards, built-in lineage, disabled deployments, device cold paths, and force-replace coverage.                                                                                                                                                                                                      |
| State storage map-store public get happy-path assertion from `ts/services/trellis/state/storage.test.ts`                                                                                               | ts/deno          | duplicate-shrunk                     | `control-plane.state-persists-across-control-plane-restart`, `state.map-store-prefix-put-get-list-delete`, `state.map-store-list-limit`, `state.value-store-create-read-delete`, `state.value-store-stale-revision-rejected`, `state.admin-inspect-and-delete-state` | shrunk | `deno task -c ts/deno.json test:service-integration -- --case control-plane.state-persists-across-control-plane-restart` passed; `deno task -c ts/deno.json test:integration -- --case state.value-store-create-read-delete` passed; `deno task -c ts/deno.json test:integration -- --case state.value-store-stale-revision-rejected` passed; `deno task -c ts/deno.json test:integration -- --case state.map-store-prefix-put-get-list-delete` passed; `deno task -c ts/deno.json test:integration -- --case state.map-store-list-limit` passed; `deno task -c ts/deno.json test:integration -- --case state.admin-inspect-and-delete-state` passed; `deno test --no-check -A -c ts/deno.json ts/services/trellis/state/storage.test.ts` passed; `deno check -c ts/deno.json ts/services/trellis/state/storage.test.ts` passed.                                                                                                                                                                                                  | Removed only the redundant map-store public `get`/`assertFound` happy-path assertion from `StateStore lists lexicographic pages for map stores`. Retained lexicographic ordering, pagination offsets/count/nextOffset, limit-zero behavior, conditional writes, unconditional put no-extra-load, TTL expiry, KV-safe encoding, dotted tuple collision prevention, sentinel-like key listing, provenance/migration metadata, corruption/non-JSON handling, validation errors, and injected KV failure paths.                                                                                         |
| Stored service resource binding happy-path assertions from `ts/services/trellis/auth/bootstrap/service.test.ts`                                                                                        | ts/deno          | duplicate-retired-live-control-plane | `control-plane.resources-survive-control-plane-restart`, `resources.service-receives-required-bindings`, `resources.service-kv-create-put-get-delete`, `resources.service-store-create-read-list-delete`                                                             | shrunk | `deno task -c ts/deno.json test:service-integration -- --case control-plane.resources-survive-control-plane-restart` passed; `deno task -c ts/deno.json test:integration -- --case resources.service-receives-required-bindings` passed; `deno task -c ts/deno.json test:integration -- --case resources.service-kv-create-put-get-delete` passed; `deno task -c ts/deno.json test:integration -- --case resources.service-store-create-read-list-delete` passed; `deno test --no-check -A -c ts/deno.json ts/services/trellis/auth/bootstrap/service.test.ts` passed; `deno check -c ts/deno.json ts/services/trellis/auth/bootstrap/service.test.ts` passed.                                                                                                                                                                                                                                                                                                                                                                    | Removed only `POST /bootstrap/service returns stored resource bindings`. Retained first-start acceptance, accepted offer creation/status/digest/staleness, contract storage, capability projection, jobs/event-consumer binding shape, missing optional bindings, stale binding tolerance, resource grants, reconciliation, rollback/purge-adjacent, and migration tests.                                                                                                                                                                                                                           |
| Service-local `JobRef` wait/cancel happy-path assertions from `ts/packages/trellis/service/runtime/service_test.ts`                                                                                    | ts/deno and rust | duplicate-retired-live-control-plane | `control-plane.jobs-admin-lists-and-cancels-job`, `jobs.keyed-jobs-serialize-same-key`, `jobs.job-wait-returns-typed-result`                                                                                                                                         | shrunk | `deno task -c ts/deno.json test:service-integration -- --case control-plane.jobs-admin-lists-and-cancels-job` passed; `deno task -c ts/deno.json test:integration -- --case jobs.keyed-jobs-serialize-same-key` passed; `deno task -c ts/deno.json test:integration -- --case jobs.job-wait-returns-typed-result` passed; `cargo test --manifest-path rust/Cargo.toml -p trellis-rs --test integration jobs_keyed_jobs_serialize_same_key -- --nocapture` passed; `cargo test --manifest-path rust/Cargo.toml -p trellis-rs --test integration jobs_job_wait_returns_typed_result -- --nocapture` passed; `deno test --no-check -A -c ts/deno.json ts/packages/trellis/service/runtime/service_test.ts ts/packages/trellis/service/runtime/internal_jobs/job-manager_test.ts ts/packages/trellis/service/runtime/internal_jobs/key-coordinator_test.ts ts/packages/trellis/service/runtime/internal_jobs/runtime-worker_test.ts` passed; `deno check -c ts/deno.json ts/packages/trellis/service/runtime/service_test.ts` passed. | Removed only `service-local JobRef wait observes scoped lifecycle events` and the duplicate final `ref.get()` cancelled-state assertion from `service-local JobRef cancel publishes scoped cancelled lifecycle event`. Updated local NATS test doubles for current `Subscription`/`NatsConnection` interfaces. Retained duplicate-handler registration, service wait worker startup/stop behavior, wait-after-terminal behavior, cancel publication event/header details, cancel-after-terminal no-op behavior, and low-level job reducer/projector/janitor/advisory/keyed-policy/worker internals. |
| Operation start/wait success assertions from `ts/packages/trellis/service/runtime/service_operation_test.ts`                                                                                           | ts/deno          | duplicate-retired-live-shared-matrix | `operations.client-starts-operation`, `operations.client-waits-for-completion`, `operations.client-watches-progress`                                                                                                                                                 | shrunk | `deno task -c ts/deno.json test:integration -- --case operations.client-starts-operation` passed; `deno task -c ts/deno.json test:integration -- --case operations.client-waits-for-completion` passed; `deno task -c ts/deno.json test:integration -- --case operations.client-watches-progress` passed; `cargo test --manifest-path rust/Cargo.toml -p trellis-rs --test integration operations_client -- --nocapture` passed; `deno test --no-check -A -c ts/deno.json ts/packages/trellis/service/runtime/service_operation_test.ts` passed; `deno check -c ts/deno.json ts/packages/trellis/service/runtime/service_operation_test.ts` passed.                                                                                                                                                                                                                                                                                                                                                                               | Removed only duplicated start-ref existence and terminal completed-output assertions from `TrellisService.operation handles owned workflows`. Updated shared routed NATS test doubles for current `Subscription`/`NatsConnection` interfaces. Retained owned workflow handler registration, service handler input assertion, client context assertion, deferred completion behavior, progress watcher behavior, cancellation/signal routing, transport/control frame handling, and operation edge/error units.                                                                                      |
| Schema-validation fake-runtime RPC success assertion from `ts/packages/trellis/tests/schema_validation_integration_test.ts`                                                                            | ts/deno          | duplicate-retired-live-shared-matrix | Existing `rpc.client-calls-service-success` plus pure schema validation coverage in `ts/packages/trellis/tests/schema_validation_error_test.ts`; no new live cases.                                                                                                  | shrunk | `deno test --no-check -A -c ts/deno.json ts/packages/trellis/tests/schema_validation_integration_test.ts ts/packages/trellis/tests/schema_validation_error_test.ts` passed; `deno check -c ts/deno.json ts/packages/trellis/tests/schema_validation_integration_test.ts ts/packages/trellis/tests/schema_validation_error_test.ts` passed; `deno task -c ts/deno.json test:integration -- --case rpc.client-calls-service-success` passed; `cargo test --manifest-path rust/Cargo.toml -p trellis-rs --test integration rpc_client_calls_service_success -- --nocapture` passed.                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Removed only `RPC valid annotated input succeeds and handler is called`. Retained invalid schema over-wire tests because they are not live-replaced, and retained pure `SchemaValidationError`/annotation parse coverage in `schema_validation_error_test.ts`.                                                                                                                                                                                                                                                                                                                                      |
...
later deleted `ts/services/trellis/auth/session/bind.test.ts` after TS and Rust
...
- `/bootstrap/client` auth-required, unknown digest, and non-client digest fake
  assertions from `ts/services/trellis/auth/bootstrap/client.test.ts`:
...
- HTTP session logout route assertions from
  `ts/services/trellis/auth/http/session_logout_routes.test.ts`: `retired`,
...
- Service deployment admin RPC assertions from
  `ts/services/trellis/auth/admin/rpc.test.ts`: `retired-live-control-plane`,
...
- Auth admin service deployment staged-validation assertions from
  ...
  coverage.
- Catalog RPC assertions from `ts/services/trellis/catalog/rpc.test.ts`:
  `retired-live-control-plane`, `ts/deno and rust`, shrunk, replaced by
  `control-plane.catalog-active-contracts-survive-restart`,
  `control-plane.catalog-dependency-issue-resolved-by-provider`,
  `control-plane.catalog-force-replace-resolves-catalog-issue`,
  ...
  `deno check -c ts/deno.json ts/services/trellis/catalog/rpc.test.ts ts/services/trellis/integration/control-plane/catalog_surface_status_reports_provider_runtime.integration_test.ts`,
  `deno test --no-check -A -c ts/deno.json ts/services/trellis/catalog/rpc.test.ts`,
  `deno task -c ts/deno.json test:service-integration -- --case control-plane.catalog-surface-status-reports-provider-runtime`,
  `deno test -A -c ts/deno.json ts/services/trellis/integration/matrix_conformance_test.ts`,
...
- Session bind/revoke helper assertions from
  `ts/services/trellis/auth/session/bind.test.ts` and
  `ts/services/trellis/auth/session/revoke_runtime_access.test.ts`:
...
- Schema-validation fake-runtime invalid RPC input assertions from
  `ts/packages/trellis/tests/schema_validation_integration_test.ts`:
  `retired-live-shared-matrix`, `ts/deno and rust`, deleted, replaced by
  ...
  `ts/packages/trellis/tests/schema_validation_error_test.ts`.
- Prepared-event fake-runtime publish/header/error-annotation assertions from
  `ts/packages/trellis/tests/prepared_events_test.ts`:
...
- Service runtime fake-NATS heartbeat baseline and duplicate `JobRef` cancelled
  state assertions from `ts/packages/trellis/service/runtime/service_test.ts`:
  ...
  `deno task -c ts/deno.json test:integration -- --case health.service-publishes-authorized-sample`,
  `deno task -c ts/deno.json test:integration -- --case health.projection-lifecycle-and-recovery`,
  `deno task -c ts/deno.json test:service-integration -- --case control-plane.jobs-admin-lists-and-cancels-job`,
  `deno task -c ts/deno.json test:integration -- --case jobs.job-wait-returns-typed-result`,
  `deno test --no-check -A -c ts/deno.json ts/packages/trellis/service/runtime/service_test.ts ts/packages/trellis/service/runtime/internal_jobs/runtime-worker_test.ts`,
  and
  `deno check -c ts/deno.json ts/packages/trellis/service/runtime/service_test.ts ts/packages/trellis/service/runtime/internal_jobs/runtime-worker_test.ts`.

Schema invalid-RPC verification bundle:
`deno task -c ts/deno.json check:integration` passed;
`deno check -c ts/deno.json ts/packages/trellis-test/src/nats_container.ts ts/packages/trellis-test/src/runtime.ts ts/packages/trellis-test/index.ts ts/packages/trellis/tests/prepared_events_test.ts ts/packages/trellis/tests/schema_validation_error_test.ts`
passed;
`deno task -c ts/deno.json test:integration -- --case rpc.invalid-annotated-input-schema-validation`
passed;
`deno task -c ts/deno.json test:integration -- --case rpc.invalid-mixed-input-validation`
passed;
...
passed;
`deno test --no-check -A -c ts/deno.json ts/packages/trellis/tests/prepared_events_test.ts ts/packages/trellis/tests/schema_validation_error_test.ts`
passed.

Prepared-events verification bundle:
`deno task -c ts/deno.json check:service-integration` passed;
`deno check -c ts/deno.json ts/packages/trellis-test/src/nats_container.ts ts/packages/trellis-test/src/runtime.ts ts/packages/trellis-test/index.ts ts/packages/trellis/tests/prepared_events_test.ts ts/packages/trellis/tests/schema_validation_error_test.ts`
passed;
`deno task -c ts/deno.json test:service-integration -- --case prepared-events.prepared-publish-preserves-custom-headers-and-annotates-handler-error`
passed;
`deno test --no-check -A -c ts/deno.json ts/packages/trellis/tests/prepared_events_test.ts ts/packages/trellis/tests/schema_validation_error_test.ts`
passed.
...
- Durable event listener fake-NATS assertions from
  `ts/packages/trellis/tests/trellis_api_guard_test.ts`:
  ...
  Verification passed:
  `deno task -c ts/deno.json test:service-integration -- --fixture event-consumers`,
  `deno task -c ts/deno.json check:service-integration`,
  `deno test --no-check -A -c ts/deno.json ts/packages/trellis/tests/trellis_api_guard_test.ts`,
  and
  `deno check -c ts/deno.json ts/packages/trellis-test/src/runtime.ts ts/packages/trellis-test/src/nats_container.ts`.
...
`transfer_test.ts`, and `request_error_test.ts` under `ts/packages/trellis/**`.
Durable event-consumer live coverage has replaced the fake JetStream/NATS
behavior in `ts/packages/trellis/tests/trellis_api_guard_test.ts`;
...
- Accidental NATS interface compatibility members were removed from remaining
  ...
  `ts/packages/trellis/client_connect_test.ts`,
  `ts/packages/trellis/device.test.ts`,
  `ts/packages/trellis/service/runtime/transfer_test.ts`,
  `ts/packages/trellis/tests/rpc_integration_test.ts`,
  `ts/packages/trellis/tests/telemetry_error_metrics_test.ts`, and
  `ts/packages/trellis/tests/trellis_api_guard_test.ts`. This is a mock-shrink
  cleanup only; no behavior assertions were retired from those files by this
  note.
- The deterministic-only remainder of
  `ts/packages/trellis/tests/prepared_events_test.ts` had the same fake NATS
...
- `ts/packages/trellis/tests/rpc_integration_test.ts` no longer retains the fake
...
- `ts/packages/trellis/service/runtime/transfer_test.ts` no longer retains the
  fake subscription-readiness assertions for `ServiceTransfer.initiateUpload`
  and `ServiceTransfer.initiateDownload`. Replaced by TS and Rust live cases
...
- Transfer fake assertions retired, 2026-06-25: deleted
  `ts/packages/trellis/service/runtime/transfer_test.ts` after the remaining
...
- Routed NATS helper retired, 2026-06-25: deleted
  `ts/packages/trellis/testing/routed_nats.ts` after no TypeScript imports
  remained.
- Operation fake assertions retired, 2026-06-25: deleted
  `ts/packages/trellis/service/runtime/service_operation_test.ts` after its
  remaining service operation facade and durable-control assertions were
  replaced by TS and Rust live rows:
  ...
  fixture/check files, Rust `operations_service_` live test group, TypeScript
  and Rust shared-matrix conformance, and TypeScript/Rust format checks.
- Operation fake assertion retired, 2026-06-25:
  `ts/packages/trellis/tests/operations_attach_it.ts` was deleted. Its
...
`deno check -c ts/deno.json ts/services/trellis/auth/account_flows/bootstrap.test.ts ts/services/trellis/auth/bootstrap/client.test.ts`.
...
migration case mirrors the TypeScript base-service-active flow, and the outbox
fixture keeps `TrellisTestRuntime` alive for the service/client lifetime. No
unit files were retired by this parity packet.
...
`ts/services/trellis/auth/session/bind.test.ts` and
`ts/services/trellis/auth/session/revoke_runtime_access.test.ts` after TS and
...
- Final required checks passed: `deno fmt -c ts/deno.json --check`,
  `deno task -c ts/deno.json check`,
  `cargo fmt --manifest-path rust/Cargo.toml --all --check`,
  `cargo check --manifest-path rust/Cargo.toml --workspace`,
  `cargo test --manifest-path rust/Cargo.toml --workspace --lib`, both
  TypeScript matrix conformance tests, and both Rust matrix conformance tests.
...
- Retained TypeScript unit groups now include in-test comments explaining why
  they remain. `ts/services/trellis/auth/bootstrap/client.test.ts` keeps exact
...
- The removed `ts/services/trellis/auth/http/session_logout_routes.test.ts` has
...
- `ts/services/trellis/auth/admin/rpc.test.ts` keeps pure normalizers, schemas,
...
- `ts/services/trellis/catalog/rpc.test.ts` keeps stale-authority binding hiding
...
- `ts/packages/trellis/tests/trellis_api_guard_test.ts` keeps API/facade guards
  that should fail before network use. Live event-consumer and RPC recovery
  behavior has moved to TS/Rust matrix rows.
- `ts/services/trellis/auth/session/rpc.test.ts` keeps deterministic session RPC
