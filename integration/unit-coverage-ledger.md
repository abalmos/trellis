...
- Server/service runtime: `ts/packages/trellis/server/**/*_test.ts`,
...
- Service runtime fake-NATS heartbeat baseline and duplicate `JobRef` cancelled
  state assertions from `ts/packages/trellis/server/service_test.ts`:
  ...
  `deno test --no-check -A -c ts/deno.json ts/packages/trellis/server/service_test.ts ts/packages/trellis/server/internal_jobs/runtime-worker_test.ts`,
  and
  `deno check -c ts/deno.json ts/packages/trellis/server/service_test.ts ts/packages/trellis/server/internal_jobs/runtime-worker_test.ts`.
...
- Accidental NATS interface compatibility members were removed from remaining
  ...
  `ts/packages/trellis/server/transfer_test.ts`,
...
- `ts/packages/trellis/server/transfer_test.ts` no longer retains the fake
...
- Transfer fake assertions retired, 2026-06-25: deleted
  `ts/packages/trellis/server/transfer_test.ts` after the remaining
...
- Operation fake assertions retired, 2026-06-25: deleted
  `ts/packages/trellis/server/service_operation_test.ts` after its remaining
...
- `ts/packages/trellis/server/service_test.ts` keeps service-local
