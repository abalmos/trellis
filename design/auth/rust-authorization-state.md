...
The TypeScript implementation is a behavioral and wire-compatibility reference
until the external cutover. It is not a second owner of the Rust records.

| Current TypeScript module                               | Current responsibility and storage                                              | Current external surface                                  | Rust target                                           | Action                                                                   |
| ------------------------------------------------------- | ------------------------------------------------------------------------------- | --------------------------------------------------------- | ----------------------------------------------------- | ------------------------------------------------------------------------ |
| `auth/storage/sessions_users_approvals.ts`              | User projections in `users`                                                     | Auth user/admin RPCs and login flows                      | `platform::auth::repository`                          | Migrate principal records; defer user/admin APIs                         |
| `auth/storage/sessions_users_approvals.ts`              | Provider identities in `user_identities`                                        | OAuth/OIDC and local-identity flows                       | provider-identity repository                          | Model linkage; defer login flows                                         |
| `auth/storage/sessions_users_approvals.ts`              | Sessions in `sessions`                                                          | bootstrap, bind, session RPCs, request validation         | session repository                                    | Migrate durable session semantics; defer APIs                            |
| `auth/session/*`                                        | Session-key binding, principal projection, revocation, and session RPC behavior | `Auth.Sessions.*`, `Auth.Requests.Validate`               | session and issuable-state services                   | Migrate state only; defer APIs and validation cutover                    |
| `auth/storage/sessions_users_approvals.ts`              | Identity authority and identity grants                                          | consent and identity-grant RPCs                           | identity-authority repository                         | Migrate accepted exact authority; defer consent APIs                     |
| `auth/storage/authority.ts`                             | Deployment desired authority and plans                                          | `Auth.DeploymentAuthority.*`                              | deployment-authority repository                       | Migrate current desired authority; defer plan/admin APIs                 |
| `auth/reconciliation/authority_reconciler.ts`           | Materialized authority and physical resource convergence                        | reconciliation RPC and bootstrap wait                     | materializer and materialization repository           | Migrate pure authority materialization; defer external provisioning APIs |
| `auth/storage/authority.ts`                             | Materialized grants and bindings                                                | bootstrap and auth callout                                | materialization repository                            | Migrate exact grants and structured evidence                             |
| `auth/bootstrap/service.ts`, `auth/bootstrap/device.ts` | Contract binding, dependency resolution, and runtime bootstrap assembly         | bootstrap HTTP routes                                     | participant binding and runtime-evidence repositories | Migrate state inputs; defer routes to the external cutover               |
| global contract storage and `auth/register.ts`          | Contract lookup and active implementation offers                                | catalog/admin/bootstrap surfaces                          | exact participant/API artifact repository             | Migrate exact artifact references; retire contract-as-authority behavior |
| `auth/storage/services.ts`                              | Service deployment and instance records                                         | service deployment/instance RPCs                          | service authorization projection                      | Model minimum eligibility evidence; defer admin APIs                     |
| `auth/storage/devices_activation.ts`                    | Device deployments, instances, activation, and review records                   | device admin and activation APIs                          | device authorization projection                       | Model deployment/instance/activation evidence; defer APIs                |
| `auth/device_activation/*`                              | Device activation and delegation workflow                                       | HTTP activation routes and `Auth.DeviceUserAuthorities.*` | activation evidence repository                        | Model resolved evidence; defer workflow to external cutover              |
| `auth/http/*`                                           | OAuth/OIDC, login, bind, logout, and bootstrap HTTP routes                      | `/auth/*`                                                 | future Rust auth service                              | Defer to external cutover                                                |
| `auth/admin/*`, `auth/approval/*`, `auth/register.ts`   | Auth admin RPC and operation registration                                       | `rpc.v1.Auth.*`, `operations.v1.Auth.*`                   | future Rust auth service                              | Defer to external cutover                                                |
| `auth/callout/callout.ts`                               | NATS auth callout and transport permission derivation                           | `$SYS.REQ.USER.AUTH`                                      | future Rust auth callout                              | Defer to external cutover; never import ACLs as durable authority        |
| request/event validation handlers                       | Transitional centralized proof validation                                       | `Auth.Requests.Validate`, `Auth.Events.Validate`          | later local runtime validation                        | Intentionally retain until their dedicated cutovers                      |
| auth flow and connection KVs                            | OAuth state, browser flows, pending binds, connection presence                  | login, activation, and connection lifecycle               | future Rust auth service/runtime presence             | Defer; these are not durable authority                                   |
