...
| Built-in browser apps            | Portal source under `ts/portals/login`; Console source under `ts/apps/console`                                                                                          | Compiled static output                                                                         | `/login/*`, `/assets/login/*`, `/console/*`                                                               | Provide same-origin login and administration without a production filesystem mount; external Console origins remain supported                                                                                                | Rust embedded-asset HTTP adapter; Svelte remains source                                                | complete                                 |
...
The Svelte login portal and Console remain the authored UIs. Repository
preparation builds their static output reproducibly into generated artifacts.
The Rust release binary embeds that output, including content-hashed assets. SPA
fallback applies only beneath `/login/*` and `/console/*`; hashed login-asset
misses and unknown auth/bootstrap/API paths remain ordinary `404` responses. The
embedded Console receives same-origin runtime configuration and bypasses the
external deployment's Trellis-origin selector. Production has no source-tree or
mounted asset dependency. A configured development override directory may
replace login portal files without changing production behavior, and
path-component checks reject traversal. The override directory is trusted
development input; its symlink layout is not a production security boundary.
...
| Browser session bind (no replacement path)                  | `rust/crates/runtime/src/platform/auth/http/browser/consent.rs::bind_flow` calls `rust/crates/runtime/src/platform/auth/application/sessions.rs::AuthService::create_session` -> `rust/crates/runtime/src/platform/auth/application/repository.rs::SessionRepository::create_session`, implemented by `rust/crates/runtime/src/platform/auth/sqlite/sessions.rs::create_session`; the bind idempotency result can replay the committed session ID. There is no session-key rebind or replacement method. | A successful new bind creates a session. Retrying the same idempotency request replays that session; it does not replace or revoke an existing session or context.                                                                                                                                      |
