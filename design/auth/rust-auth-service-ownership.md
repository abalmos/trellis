...
Legacy paths were relative to the removed `js/services/trellis` tree unless
stated otherwise.
...
| NATS Auth Callout                | `auth/callout/callout.ts`                                                                                                                                               | `$SYS.REQ.USER.AUTH`; signing/XKey material                                                    | NATS auth request/response JWT                                                                | Verify the NATS challenge and digest-selected signed context, compile exact permissions directly, issue bounded JWT, seal response, and fail safely                                                                          | `platform::auth::callout` under `platform.owner`                                                       | migrate                                  |
