...
service contracts expose participant JSON/digests, owned API JSON/digest, and
exact referenced API JSON/digest pairs for bootstrap.
...
Generated API crates expose native API constants. Generated participant and
service crates expose `CONTRACT_ID`, `CONTRACT_DIGEST`, participant JSON,
`PARTICIPANT_NEEDS_DIGEST`, owned API evidence, and referenced API evidence. Run
`cargo xtask prepare` after source changes; generated files are not authoring
inputs.
