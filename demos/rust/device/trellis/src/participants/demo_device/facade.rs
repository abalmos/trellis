...
impl<'a> Client<'a> {
    ...
    /// Access the `fieldOps` dependency alias facade.
    pub fn field_ops(&self) -> uses::field_ops::Client<'a> {
        uses::field_ops::Client::new(self.inner)
    }
}
impl<'a> Service<'a> {
    ...
    /// Access the `fieldOps` dependency alias facade for outbound calls.
    pub fn field_ops(&self) -> uses::field_ops::Client<'a> {
        uses::field_ops::Client::new(self.inner)
    }
}
