...
const jsRoot = fromFileUrl(new URL("../", import.meta.url));
...
function parseArgs(args: readonly string[]): Options {
  let inputDir = resolve(jsRoot, "coverage/browser");
  let output = resolve(jsRoot, "coverage/browser/lcov.info");
  let buildDir = resolve(jsRoot, "portals/login/build");
...
function resolvePath(path: string): string {
  return isAbsolute(path) ? path : resolve(jsRoot, path);
}
